...
def send_car_commands():
    while True:
        ...
        if LiveValues.car_velocity < dead_zone_size and LiveValues.car_velocity > -dead_zone_size:
            if dry_run:
            ...
            if dry_run:
                if LiveValues.iteration % log_rate == 0: print(f'''go: {LiveValues.car_steer}, {LiveValues.car_velocity}''')
            else:
                Car.drive(velocity=LiveValues.car_velocity, direction=direction)
        
        # return back torwards zeros if no new command given
        if LiveValues.car_velocity < 0:
            LiveValues.car_velocity += speed_neutralize_rate
        if LiveValues.car_velocity > 0:
            LiveValues.car_velocity -= speed_neutralize_rate
    ...
    async for message in websocket:
        try:
            if websocket.path != "/keypresses":
                ...
                LiveValues.car_velocity = json_message["velocity"]
                LiveValues.car_steer    = json_message["direction"]
        except Exception as error:
            print(f'''Error when handling websocket message: {error}''')
