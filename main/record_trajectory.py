...
try:
            ...
            try:
                ...
                with connect("ws://localhost:8080") as websocket:
                    ...
                    while 1:
                        ...
                        if "ERR" in message:
                            if "Could not claim interface" in message:
                                print(f'''\n\n\n    - VR issue: make sure nothing else is listening to the VR websocket (e.g. webview or duplicate process)\n''')
                                thread.interrupt_main()
                            else:
                                print(f'''\n\n\n    - VR tracker error: {message}\n''')
    ...
    # connection to car
    # 
    car_process_log_file = open(f"{logging_folder}/car_process.log", "w+")
    ...
    def send_action():
        ...
        while True:
            if car_address == None or car_port == None:
                time.sleep(5)
                continue
            car_process = subprocess.Popen(["ssh", f"ubuntu@{car_address}", "-t", f"cd ~/repos/valve_car_tracker/ && python3 ./repos/car/drive_listener.py ip_address:{car_address}"], stdout=car_process_log_file, stderr=car_process_log_file, stdin=subprocess.DEVNULL)
    ...
    if os.getenv("VIRKSHOP_HOME") == None:
        # FIXME: more of just-for-debugging
        ...
        pid_path = f"{logging_folder}/survive-websocketd.pid"
        try:
        ...
        if output:
            subprocess.Popen(["sudo", "kill", output.strip(),], stdin=subprocess.DEVNULL, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
            time.sleep(1)
        
        # start the new process
        process = subprocess.Popen(["sudo", "-E", "env", f"PATH={os.getenv('PATH')}", "survive-websocketd",], stdout=vr_process_log_file, stderr=vr_process_log_file, stdin=subprocess.DEVNULL) 
        with open(pid_path, 'w') as the_file:
            the_file.write(str(process.pid))
    
    time.sleep(2)
    position_should_be_listening = True
    while position == None:
        print(f'''waiting for VR tracker to connect (make sure plugged in and on, sometimes it takes 2min)''')
        time.sleep(10)
    print(f'''vr connected!''')
